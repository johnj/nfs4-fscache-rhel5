...
static inline void dput_path(struct vfs_path *path, struct nameidata *nd)
{
	dput(path->dentry);
	if (path->mnt != nd->mnt)
		mntput(path->mnt);
...
static inline void path_to_nameidata(struct vfs_path *path, struct nameidata *nd)
{
	dput(nd->dentry);
	if (nd->mnt != path->mnt)
		mntput(nd->mnt);
 ...
 */
static inline int do_follow_link(struct vfs_path *path, struct nameidata *nd)
{
	int err = -ELOOP;
 ...
 *   were called with.
 */
static int follow_automount(struct vfs_path *path, unsigned flags,
			    bool *need_mntput)
{
	struct vfsmount *mnt;
 ...
 */
static int managed_dentry(struct vfs_path *path, unsigned flags)
{
	unsigned d_managed;
	bool need_mntput = false;
	int ret;

	while (d_managed = path->dentry->d_mounted,
 ...
 */
int __follow_down(struct vfsmount **mnt, struct dentry **dentry, bool mounting_here)
{
	unsigned d_managed;
	int ret;

	while (d_managed = (*dentry)->d_mounted,
	       unlikely(d_managed != 0)) {
		/* Allow the filesystem to manage the transit without i_mutex
		 * being held.
		 *
...
static int do_lookup(struct nameidata *nd, struct qstr *name,
		     struct vfs_path *path, int atomic)
{
